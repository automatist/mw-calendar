...
=== Upgrade FYI (ReadMe!) ===
If your upgrading from earler versions of this extension, please read the comments below as changes may need to be addressed post upgrade.

* v3.5.0.1 - Displaying events created in v3.0 are now disabled by default. Doing lookups for the old format and new format was a performace drag and I decided to disable it by default. You can re-enable this old style lookup by adding <font color=red>'''enablelegacy'''</font> to you config/parameters.
** '''old''': "Public (1-6-2009) - Event 1" ''(v3.0)''
**'''new''': "Main Page/Public/1-21-2009 -Event 1" ''(v3.2 and newer)''

* v3.6 - I've introduced iCal imports, however its very basic at this time. It may even be useless as it doesn't load repeating events like holidays. It will load single events such as appointments, meetings or any one time event.
